...
# -*- coding: utf-8 -*-
# Streamlitアプリケーション: Trace Log Analysis Dashboard (Modified Logic)
# 修正内容: QRコードがない場合（空欄/nan）はQRresultをOK扱いとし、TESTresultの結果のみでOK/NG判定を行います。
    ...
    try:
        ...
        for col in target_cols:
            if col in df.columns:
                # 文字列化して空白除去。NaNは 'nan' という文字列になる可能性があります
        ...
        def determine_status(row):
            ...
            # QR判定: 'OK' または 'データなし(空文字/NAN)' の場合は合格扱いとする
            is_qr_ok = False
            if qr == 'OK':
                is_qr_ok = True
            elif qr in ['', 'NAN', 'NONE']:  # QRがない場合の条件
                is_qr_ok = True
            
            # 最終判定: QRが合格扱い かつ TESTがOK なら合格
            if is_qr_ok and test == 'OK':
                return 'OK'
            else:
                return 'NG'
        ...
        def determine_ng_reason(row):
            ...
            # QRが明示的に 'NG' の場合のみ、QRエラーとする
            # (空文字やnanはOK扱いなので、ここには引っかからない)
            if qr == 'NG':
                return 'QR_NG'
            
            # QRがOK(またはなし)で、TESTがNGの場合
            elif test == 'NG':
