...
# --- 修正: サイドバーの期間選択を年(プルダウン)と月(チェックボックスグリッド)に変更 ---2025-01-09再修正
# --- 修正: 年プルダウンの全期間を削除し、全期間ボタンを新設 ---2025-01-09追加修正
# --- 修正: 全期間ボタン押下時に月のチェックボックスも全選択状態にする ---2025-01-09追加修正2
# --- 修正: Session State APIのエラー対応 (初期化ロジックの変更) ---2025-01-09追加修正3
...
CSV_FILENAME = '20260121E61_Trace_log.csv'
...
if df is not None:
    ...
    # --- State管理: 全期間表示フラグ と 月のチェック状態 ---
    if 'force_all_period' not in st.session_state:
        st.session_state.force_all_period = False

    # --- 修正: 月のチェックボックスの初期化 ---
    # ここで事前にsession_stateに値を入れておくことで、
    # widget生成時に value=... を指定しなくてもデフォルト値として機能させます。
    # こうすることで "The widget with key ... was created with a default value..." エラーを回避します。
    for m in range(1, 13):
        key = f"month_{m}"
    ...
    def on_click_all_period():
        st.session_state.force_all_period = True
        # ボタンが押されたら1月〜12月のチェックボックスを全てTrueにする
        for m in range(1, 13):
            st.session_state[f"month_{m}"] = True

    def on_change_filter():
        # フィルター(年・月・FCT)が操作されたら全期間モードを解除
        st.session_state.force_all_period = False

    # --- 全期間表示ボタン ---
    st.sidebar.button("全期間データを表示", on_click=on_click_all_period, type="primary")

    st.sidebar.markdown("---")

    # --- 年の選択 (プルダウン) ---
    year_options = [str(y) for y in range(2025, 2031)]
    selected_year = st.sidebar.selectbox("年 (Year)", year_options, on_change=on_change_filter)

    # --- 月の選択 (チェックボックスのグリッド配置) ---
    st.sidebar.write("月 (Month)")
    ...
    for m in range(1, 13):
        ...
        with cols[col_idx]:
            # --- 修正: value引数を削除 ---
            # session_stateに既にキーがあるため、value=True と書くとエラーになります。
            # Streamlitは自動的に session_state[key] の値を表示に使用します。
            if st.checkbox(f"{m}月", key=f"month_{m}", on_change=on_change_filter):
                selected_month_nums.append(m)
    ...
    for fct in unique_fcts:
        if st.sidebar.checkbox(fct, value=True, key=f"fct_{fct}", on_change=on_change_filter):
            selected_fcts.append(fct)

    # --- フィルタリング処理 ---
    df_filtered = df.copy()
    
    # 全期間ボタンが押されているかどうかで分岐
    if st.session_state.force_all_period:
        # 全期間モード: 年と月のフィルタリングをスキップ
        st.sidebar.success("現在: 全期間データを表示中")
        period_info = "全期間データ (All Data)"
        ...
        # 通常モード: 年・月・FCTでフィルタリング
        
        # 1. 年フィルター
        df_filtered = df_filtered[df_filtered['Year_str'] == selected_year]
            
        # 2. 月フィルター
        if selected_month_nums:
            df_filtered = df_filtered[df_filtered['Month_int'].isin(selected_month_nums)]
        else:
            df_filtered = df_filtered[0:0]
        ...
        if len(selected_month_nums) == 12:
            ...
            month_info = ",".join([str(m) for m in selected_month_nums]) + "月"
        period_info = f"{selected_year} / {month_info}"

    st.info(f"表示対象: {period_info} / データ件数: {len(df_filtered)} 件")
