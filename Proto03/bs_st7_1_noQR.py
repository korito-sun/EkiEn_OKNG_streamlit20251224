...
if df is not None:
    ...
    # --- Session State 初期化 (日付範囲用) ---
    # date_input の key='filter_dates' で管理するため、
    # 初期値としてタプル (start, end) を設定する
    ...
    # --- 日付範囲選択 (カレンダー) ---
    # key='filter_dates' を指定することで、値が自動的に Session State に保存・読込されます
    selected_dates = st.sidebar.date_input(
        "期間選択 (Start - End)",
        value=st.session_state.filter_dates,
    ...
    # 1. 日付範囲フィルター
    # date_input は範囲選択時、(start, end) のタプルを返すが、
    # 選択途中では (start,) だけの場合があるため長さをチェック
    if isinstance(selected_dates, tuple) and len(selected_dates) == 2:
        start_d, end_d = selected_dates
        
        # DataFrameのDate列と比較してフィルタリング
        # (Date列は datetime.date オブジェクト)
        mask = (df_filtered['Date'] >= start_d) & (df_filtered['Date'] <= end_d)
        df_filtered = df_filtered.loc[mask]
        
        period_info = f"{start_d} ～ {end_d}"
    else:
        # 範囲が確定していない場合（片方選択時など）
        # ここでは便宜上、選択中の開始日などを表示しつつ全データまたは空にする制御も可能だが
        # エラー防止のため元のデータを維持または警告を出す
