...
if df is not None:
    ...
    for m in range(1, 13):
        col_idx = (m - 1) % 3
        with cols[col_idx]:
            # デフォルトですべてチェック(True)にしておく
            # on_changeイベントを追加して、操作時に全期間モードを解除
            if st.checkbox(f"{m}月", value=True, key=f"month_{m}", on_change=on_change_filter):
    ...
    if st.session_state.force_all_period:
        ...
        # FCTフィルターは常に適用するのが自然ですが、"全データ"の意味合いによっては外すこともあります。
        # ここでは「期間の全データ」と解釈し、FCTフィルターは有効のままとします。
        # もしFCTも含めて全解除したい場合は、以下のFCTフィルタブロックもifの外に出すか調整してください。
        if selected_fcts:
