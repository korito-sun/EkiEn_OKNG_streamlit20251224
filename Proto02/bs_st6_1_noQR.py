...
if df is not None:
    ...
    # --- 年の選択 (プルダウン) ---
    year_options = ["全期間"] + [str(y) for y in range(2025, 2031)]
    selected_year = st.sidebar.selectbox("年 (Year)", year_options)

    # --- 修正: 月の選択 (チェックボックスのグリッド配置) ---
    st.sidebar.write("月 (Month)")
    
    # 選択された月の数字を格納するリスト
    selected_month_nums = []
    
    # 3列のカラムを作成して1~12月を配置 (画像のような3列配置を想定)
    ...
    for m in range(1, 13):
        ...
        with cols[col_idx]:
            ...
            if st.checkbox(f"{m}月", value=True, key=f"month_{m}"):
